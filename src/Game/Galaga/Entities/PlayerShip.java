    ...
    public void tick() {
        ...
        if (destroyed){
            ...
            if ((handler.getKeyManager().keyJustPressed(KeyEvent.VK_ENTER) || handler.getKeyManager().keyJustPressed(KeyEvent.VK_SPACE)) && !attacking){
                ...
                attacking = true;
                handler.getGalagaState().entityManager.entities.add(new PlayerLaser(this.x + (width / 2), this.y - 3, width / 5, height / 2, Images.reflectedLaser, handler, handler.getGalagaState().entityManager));
