	...
	public void tick() {
		...
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_EQUALS)){
			if (linking){
				...
				if(selectedList.equals(Images.moveTiles)) {
					if (counter == 4) {
						counter = 0;
					} else {
						counter++;
		...
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_B)) {
			// counter = tile we are on, selector == 0: only list with 29, instead of 41

			this.counter = this.selectedList.size()/2;
//			BufferedImage old = this.selectedList.get(this.selectedList.size()/2);
//			this.selectedList.set(this.selectedList.size()/2, this.selectedList.get(current));
//			this.selectedList.set(current, old);

		}

		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_R)) {
			this.counter = new Random().nextInt(this.selectedList.size());
		}

		if (handler.getKeyManager().shift && handler.getKeyManager().keyJustPressed(KeyEvent.VK_R)) {
			switch (new Random().nextInt(6)) {
			case 0:
				this.selectedList = Images.zeldaTiles;
				break;
			case 1:
				this.selectedList = Images.forestTiles;
				break;
			case 2:
				this.selectedList = Images.caveTiles;
				break;
			case 3:
				this.selectedList = Images.mountainTiles;
				break;
			case 4:
				this.selectedList = Images.graveTiles;
				break;
			case 5:
				this.selectedList = Images.moveTiles;
				break;
