    ...
    public void tick() {
        ...
        if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_L)){
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                handler.changeState(new ZeldaMMGameState(handler,MapBuilder.createMap(ImageIO.read(chooser.getSelectedFile()), handler,chooser.getSelectedFile().getName())));
        ...
        uiManager.addObjects(new UIImageButton((handler.getWidth() / 2) - (handler.getWidth() /3) + 24, (handler.getHeight() /2)-(handler.getHeight() /32), handler.getWidth()/7, handler.getHeight()/6, Images.galagaLogo, new ClickListlener() {
            ...
            public void onClick() {
                if (handler.getState() == handler.getGameState()) {
                	handler.getGalagaState().refresh();
                    handler.getMouseManager().setUimanager(null);
                    handler.getMusicHandler().changeMusic("CorneriaMenu.wav");
