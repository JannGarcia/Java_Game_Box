...
import Game.Zelda.Entities.Statics.MoveTile;
import Main.Handler;
...
public class MapBuilder {
	...
	public static Map createMap(BufferedImage mapImage, Handler handler,String name){
		...
		// Fix space errors
		if (path2.contains("%20")) {path2 = path2.replace("%20", " ");}
		...
		for (int i = 0; i < mapImage.getWidth(); i++) {
			for (int j = 0; j < mapImage.getHeight(); j++) {
				...
				if(currentPixel == DDDoor){
					...
					MoveTile ghost = new MoveTile(xPos, yPos, Images.moveTiles.get(0), handler, Direction.UP);
					mapInCreation.addBlock(ghost);		
				}else if(currentPixel == moveR) {
					MoveTile ghost = new MoveTile(xPos, yPos,  Images.moveTiles.get(1), handler, Direction.RIGHT);
					mapInCreation.addBlock(ghost);				
				}else if(currentPixel == moveD) {
					MoveTile ghost = new MoveTile(xPos, yPos,  Images.moveTiles.get(2), handler, Direction.DOWN);
					mapInCreation.addBlock(ghost);					
				}else if(currentPixel == moveL) {
					MoveTile ghost = new MoveTile(xPos, yPos,  Images.moveTiles.get(3), handler, Direction.LEFT);
					mapInCreation.addBlock(ghost);					
				}
	...
	public static BufferedImage arrayToRGBImage(ArrayList<ArrayList<BufferedImage>> info,String name,ArrayList<ArrayList<int[]>> teleportList){
		...
		for (int y = 0; y < info.get(0).size(); y++) {
			for (int x = 0; x < info.size(); x++) {
				...
				}else if (Images.moveTiles.get(0).equals(info.get(x).get(y))){
					image.setRGB(x,y,moveU);
				}else if (Images.moveTiles.get(1).equals(info.get(x).get(y))){
					image.setRGB(x,y,moveR);
				}else if (Images.moveTiles.get(2).equals(info.get(x).get(y))){
					image.setRGB(x,y,moveD);
				}else if (Images.moveTiles.get(3).equals(info.get(x).get(y))){
					image.setRGB(x,y,moveL);
	...
	//change this for size of blocks
	public static int pixelMultiplier = ZeldaMapMakerState.pixelsPerSquare;

	public static int Link = new Color(0, 255, 0).getRGB();

	// Move tiles
	
	public static int moveU = new Color(255, 135, 221).getRGB();
	public static int moveR = new Color(255, 56, 255).getRGB();
	public static int moveD = new Color(255, 66, 188).getRGB();
	public static int moveL = new Color(255, 91, 252).getRGB();
